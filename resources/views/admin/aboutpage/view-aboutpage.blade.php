            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">View About Us Section </li>
                    </ol>
                </div>
            </div>
        </div>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <table id="example1" class="table table-bordered table-striped" style="overflow-x: auto;">
                                        ...
                                        <tbody> 
                                        @foreach($aboutpage as $row) 
                                        <tr>
                                            <td>{{ $loop->index+1 }}</td>
                                            <td>{!! Str::limit($row->description,20)!!}</td>
                                            <td>{{date('d F Y', strtotime($row->updated_at)) }}</td>
                                            <td>
                                                <a class="btn btn-default" id="editAbout" href="{{ url('/admin/edit-aboutpage/'.$row->id) }}" title="Edit About">
